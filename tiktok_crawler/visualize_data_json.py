...
Đọc data.json → vẽ 4 biểu đồ:
  ...
  4. Pie: Phân bổ video theo Thành phố

Lưu ra:
  - chart_heatmap.png
  - chart_by_mon.png
  - chart_stacked.png
  - chart_pie_location.png
  - chart_all.png  (4 biểu đồ ghép 1 file)

Chạy: python visualize_data.py [đường_dẫn_data.json]
Mặc định: ./data.json
"""
...
def build_matrix(data: list[dict]):
    ...
    for r in data:
        ...
        loc_label = location if location in LOCATIONS_ORDER else location
...
def plot_heatmap(matrix: dict, out_path: str):
    ...
    im = ax.imshow(arr, cmap=plt.cm.YlOrRd, aspect="auto", vmin=0)
    ax.set_xticks(range(len(locs)));  ax.set_xticklabels(locs, fontsize=10, color=TEXT_COLOR)
    ax.set_yticks(range(len(mons)));  ax.set_yticklabels(mons, fontsize=9, color=TEXT_COLOR)
...
def plot_stacked(matrix: dict, out_path: str):
    ...
    mons = sorted(mons, key=lambda m: sum(matrix[m][l] for l in locs), reverse=True)
    ...
    ax.set_title("🏙️  Video theo Món & Thành phố (stacked)", fontsize=13,
                 color=ACCENT, fontweight="bold", pad=14)
    ax.legend(loc="upper right", framealpha=0.25, labelcolor=TEXT_COLOR,
...
def plot_pie_location(matrix: dict, out_path: str):
    locs   = LOCATIONS_ORDER
    totals = [sum(matrix[m][loc] for m in HASHTAG_CHINH.values()) for loc in locs]
...
def plot_all(matrix: dict, out_path: str):
    ...
    fig.suptitle("TikTok Food Crawler — Phân tích dữ liệu thô (data.json)",
...
def print_summary(matrix: dict, data: list):
    ...
    if total_all:
        print(f"  Đã duyệt (≤30 ngày): {total_duyet} ({total_duyet/total_all*100:.1f}%)")
    print("="*65 + "\n")
...
def main():
    meta_path = sys.argv[1] if len(sys.argv) > 1 else "data.json"
    ...
    print(f"\n{'='*50}")
    print(f"  TikTok Food Crawler — Visualize Data")
    print(f"{'='*50}\n")
    ...
    plot_heatmap      (matrix, str(out_dir / "chart_heatmap_data.png"))
    plot_by_mon       (matrix, str(out_dir / "chart_by_mon_data.png"))
    plot_stacked      (matrix, str(out_dir / "chart_stacked_data.png"))
    plot_pie_location (matrix, str(out_dir / "chart_pie_location_data.png"))
    plot_all          (matrix, str(out_dir / "chart_all_data.png"))
...
✅ Xong! Các file ảnh:
   ...
   • chart_all.png            — Tất cả ghép 1 file
""")
